...
static inline void interpret_rtl_lm(rtlreg_t *dest, const vaddr_t* addr, int len) {
  *dest = vaddr_read(*addr, len);
}

static inline void interpret_rtl_sm(const vaddr_t* addr, const rtlreg_t* src1, int len) {
  wp_access_mem = true;
...
static inline void rtl_rol(rtlreg_t *dest, rtlreg_t *src, int times, int width) {
  if (width == 1) {
    uint8_t val;
    val = *((uint8_t *)src);
    *dest = (val << times) | (val >> (8 - times));
  } else if (width == 2) {
    uint16_t val;
    val = *((uint16_t *)src);
    *dest = (val << times) | (val >> (16 - times));
  } else {
    uint32_t val;
    val = *((uint32_t *)src);
    *dest = (val << times) | (val >> (32 - times));
  }
}

// #include "isa/rtl.h"   FIXME:两个头文件相互包含
